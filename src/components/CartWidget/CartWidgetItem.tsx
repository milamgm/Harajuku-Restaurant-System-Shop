...
const CartWidgetItem = ({ id, quantity }) => {
  const { removeItem, incrementQuantity, decrementQuantity, products } = useCartContext();
  ...
  return (
    <>
      ...
      <div>
        <button className="b-2 btn-primary" onClick={() => decrementQuantity(id)}>
          -
        </button>
        <span className="m-5">{quantity}</span>
        <button className="b-2 btn-primary" onClick={() => incrementQuantity(id)}>
          +
        </button>
        <button
